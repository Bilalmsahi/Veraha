/**
 * Shared side panel for mapping controls to an entity (policy, document, etc.).
 * Matches the policy "Map controls to your policy" UI: Sheet, search, Framework / Framework code / Domain filters, control cards with +/Remove.
 */
...
export type MapControlsSheetProps = {
  open: boolean;
  ...
  onRemoveControl: (controlId: string, e: React.MouseEvent) => void;
  isPending: boolean;
};
          ...
          <ScrollArea className="flex-1 min-h-0 -mr-4 pr-4">
            <div className="space-y-2 pb-4">
              {controls.isLoading ? (
                ...
                controlList.map((c) => (
                  <MapControlsSheetCard
                    key={c._id}
                    control={c}
                    isMapped={alreadyMapped.has(c._id)}
                    onAdd={() => onAddControl(c._id)}
                    onRemove={(e) => onRemoveControl(c._id, e)}
                    isAdding={isPending}
                  />
                ))
              )}
...
type MapControlsSheetCardProps = {
...
function MapControlsSheetCard({
  control,
  isMapped,
  onAdd,
  onRemove,
  isAdding,
}: MapControlsSheetCardProps) {
  const [expanded, setExpanded] = useState(false);
  const desc = control.description ?? '';
  const needsTruncate = desc.length > DESCRIPTION_PREVIEW_LEN;
  const displayDesc =
    needsTruncate && !expanded ? desc.slice(0, DESCRIPTION_PREVIEW_LEN) + '…' : desc;
