...
import { usePolicyComments, useCreatePolicyComment } from '@/api/policies';
import { formatDateTime } from '@/lib/formatters';
import { MessageSquare, Send } from 'lucide-react';

type PolicyCommentsPanelProps = {
  policyId: string;
};

export function PolicyCommentsPanel({ policyId }: PolicyCommentsPanelProps) {
  const [content, setContent] = useState('');
  const { data, isLoading } = usePolicyComments(policyId);
  const createComment = useCreatePolicyComment(policyId);
